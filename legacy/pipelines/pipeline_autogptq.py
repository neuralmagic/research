...
parser = argparse.ArgumentParser(description = "Apply autogptq")

parser.add_argument("--project-name", type=str)
parser.add_argument("--task-prefix", type=str)
parser.add_argument("--pipeline-name", type=str, default="autogptq")
...
oneshot_task_id = Task.get_task(project_name="Automation",task_name="autogptq_oneshot", task_filter={'order_by': ["-last_update"]}).id

oneshot_step_name = f"{task_prefix}_autogptq"
...
if engine == "vllm":
    ...
    alpacaeval_task_id = Task.get_task(project_name="Automation",task_name="alpacaeval_vllm", task_filter={'order_by': ["-last_update"]}).id
    alpacaeval_override_engine = {
        "Args/build_vllm": args["build_vllm"],
        "Args/enable_chunked_prefill": True,
    }
else:
...
alpacaeval_override = {
    ...
    "Args/clearml_model": True,
    "Args/packages": evaluation_packages,
    "Args/annotator": "llama-2-70b-chat",
    "Args/max-new-tokens": 2048,
}
alpacaeval_override.update(alpacaeval_override_engine)

if "alpacaeval" in args["benchmark_tasks"]:
    alpacaeval_step_name = f"{oneshot_step_name}_alpacaeval_{engine}"
    pipe.add_step(
        name=alpacaeval_step_name,
        parents=[oneshot_step_name],
        base_task_id=alpacaeval_task_id,
        execution_queue=args["evaluation_queue"],
        parameter_override=alpacaeval_override,
    )
