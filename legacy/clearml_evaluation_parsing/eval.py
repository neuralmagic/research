...
def main(cfg: DictConfig) -> Tuple[List[Trainer], pd.DataFrame]:
    ...
    return trainers, eval_gauntlet_df, models_df
...
def write_results(eval_gauntlet_df, models_df):
    now = datetime.now()

    # Format the date and time as a string
    timestamp = now.strftime("%Y%m%d_%H%M%S")

    # Create a string with the timestamp in its name
    summary_filename = f"results_summary_{timestamp}.json"
    eval_gauntlet_df.to_json(summary_filename, default_handler=str)
    
    detailed_filename = f"results_detailed_{timestamp}.json"
    models_df.to_json(detailed_filename, default_handler=str)
...
if __name__ == '__main__':
    ...
    _, eval_gauntlet_df, models_df = main(cfg)
    write_results(eval_gauntlet_df, models_df)
